...
interface UsageDataRoutes {
     ...
     */
    @GET("$BASE_PATH/get")
    suspend fun getUsageData(@Query("date") query: Long): Call<MTUsageData>?
...
//    fun getAllUsageData(): Call<MTUsageData>?
    ...
    suspend fun insertUsageData(@Query("date") query: Long, @Body usage: MTUsageData): Boolean?
    @POST("$BASE_PATH/insert")
    suspend fun insertUsageData(@Query("date") query: Long, @Body usage: ByteArrayInputStream): Boolean?
